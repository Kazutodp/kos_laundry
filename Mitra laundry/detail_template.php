...
try {
    ...
    $stmt_services = $pdo->prepare("SELECT * FROM `mitra_layanan` WHERE `mitra_id` = ? ORDER BY `id` ASC");
    $stmt_services->execute([$mitra['id']]);
    $layanan_list = $stmt_services->fetchAll(PDO::FETCH_ASSOC);
...
if ($is_logged_in) {
    try {
        $stmt_addr = $pdo->prepare("SELECT alamat FROM users WHERE id = ?");
        $stmt_addr->execute([$_SESSION['user_id']]);
        $user_data = $stmt_addr->fetch();
        if ($user_data) {
...
// Determine reason for being closed (for display purposes)
$tutup_reason = '';
if (!$status_buka) {
    if ($mitra['status_buka'] == 0) {
        $tutup_reason = 'Dinonaktifkan oleh pengelola';
    } else {
        $tutup_reason = 'Di luar jam operasional';
    }
}
                    ...
                    <div class="text-white">
                        ...
                        <div class="flex items-center gap-sm">
                            ...
                            <?php else: ?>
                                ...
                                <?php if ($tutup_reason): ?>
                                    <span class="bg-white/20 backdrop-blur-md text-white/80 px-xs py-1 rounded-lg text-[11px]"><?= htmlspecialchars($tutup_reason); ?></span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-xl">
        ...
        <div class="lg:col-span-2 space-y-xl">
            <div>
                ...
                <!-- Tabs -->
                <div class="flex p-1 bg-surface-container rounded-2xl mb-lg overflow-x-auto gap-1 select-none">                    <?php
                    $has_self = false;
                    $has_kiloan = false;
                    $has_satuan = false;
                    $has_express = false;
                    ...
                    ?>
                    <?php $first = true; foreach ($tabs_to_show as $tab_id => $tab_label): ?>
                    ...
                    <?php if (!empty($mitra['fasilitas']) || !empty($mitra['keunggulan_lainnya'])): ?>
                        <button id="tab-facility" onclick="switchTab('facility')" 
                                class="tab-btn flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 whitespace-nowrap active:scale-95 <?= empty($tabs_to_show) ? 'bg-primary text-white shadow-md shadow-primary/10 font-bold' : 'text-on-surface-variant hover:text-primary hover:bg-white/50'; ?>">
                        ...
                        <?php foreach ($layanan_list as $l): ?>
                            <?php if ($l['kategori'] === $cat_id): ?>
                                ...
                                if ($l['kategori'] === 'self') {
                                    ...
                                    $unit = 'pcs';
                                } elseif (strpos(strtolower($nama_mitra), 'shoes') !== false || strpos(strtolower($nama_mitra), 'sepatu') !== false) {
                                    $unit = 'pasang';
                                }
                                ?>
                                <div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant hover:shadow-md transition-shadow flex flex-col justify-between">
                                    <div>
                                        <div class="flex justify-between items-start mb-sm">
                                            <h3 class="font-headline-md text-[20px] text-on-surface"><?= htmlspecialchars($l['nama_layanan']); ?></h3>
                                            <span class="text-primary font-bold text-lg whitespace-nowrap">Rp <?= number_format($l['harga'], 0, ',', '.'); ?>/<?= $unit; ?></span>
                                        </div>
                                        <p class="text-on-surface-variant text-body-md mb-lg"><?= htmlspecialchars($l['detail']); ?></p>
                                    </div>
                                    <button onclick="openOrderModal('<?= htmlspecialchars($l['nama_layanan']); ?>', <?= $l['harga']; ?>, '<?= $unit; ?>')" class="w-full bg-primary text-on-primary py-sm rounded-xl font-bold flex items-center justify-center gap-sm active:scale-[0.98] transition-transform">
...
<footer class="w-full bg-slate-950 text-slate-400 border-t border-slate-900 mt-xl">
    <div class="max-w-7xl mx-auto px-lg py-xl grid grid-cols-1 md:grid-cols-4 gap-xl">
        ...
        <div class="space-y-md">
            ...
            <ul class="space-y-sm text-sm">
                <li>
                    <a href="../layanan/layanan.php?service=baju" class="hover:text-primary transition-all duration-300 hover:translate-x-1 inline-block">Laundry Kiloan</a>
                </li>
                <li>
                    <a href="../layanan/layanan.php?service=satuan" class="hover:text-primary transition-all duration-300 hover:translate-x-1 inline-block">Laundry Satuan</a>
                </li>
                <li>
                    <a href="../layanan/layanan.php?service=sepatu" class="hover:text-primary transition-all duration-300 hover:translate-x-1 inline-block">Cuci Sepatu Premium</a>
                </li>
                <li>
                    <a href="../layanan/layanan.php?service=express" class="hover:text-primary transition-all duration-300 hover:translate-x-1 inline-block">Express Delivery</a>
                </li>
            </ul>
        </div>
    ...
    <div class="bg-surface-container-lowest w-full max-w-md rounded-xl shadow-xl overflow-hidden flex flex-col">
        ...
        <div class="p-lg space-y-lg overflow-y-auto max-h-[70vh]">
            ...
            <div id="address-container" class="space-y-xs">
                ...
                <textarea id="order-address" class="w-full bg-surface-container border border-outline-variant rounded-lg px-md py-sm focus:ring-1 focus:ring-primary outline-none h-20 resize-none text-sm" placeholder="Masukkan alamat lengkap penjemputan/pengantaran..."><?= htmlspecialchars($user_address); ?></textarea>
            </div>
            <?php else: ?>
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (data.flow === 'timbang_dulu') {
                    ...
                    window.location.href = '../user/riwayat_pesanan.php';
                } else {
