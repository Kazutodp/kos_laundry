...
<head>
    ...
    <title>Kelola Pesanan (Laravel) | MataramWash Admin</title>
...
<aside class="hidden lg:flex flex-col h-screen sticky top-0 p-md space-y-md bg-slate-900 border-r border-slate-800 w-64 shrink-0 text-slate-300">
    <div class="flex items-center gap-xs px-xs py-sm border-b border-slate-800">
        <img alt="MataramWash Logo" class="h-8 w-8 object-contain brightness-110 filter" src="{{ asset('Logo_MataramWash.png?v=3') }}">
    ...
    <div class="flex flex-col gap-xs py-md border-b border-slate-800">
        ...
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/dashboard_admin.php">
            <span class="material-symbols-outlined text-[20px]">dashboard</span>
            <span class="text-label-md font-label-md">Dashboard</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/manajemen_mitra.php">
            <span class="material-symbols-outlined text-[20px]">group</span>
            <span class="text-label-md font-label-md">Manajemen Mitra</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm bg-blue-600 text-white rounded-xl font-bold border-l-4 border-blue-400 shadow-lg shadow-blue-900/30 transition-all duration-200" href="{{ route('admin.orders.index') }}">
        ...
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/operational_area.php">
            <span class="material-symbols-outlined text-[20px]">map</span>
            <span class="text-label-md font-label-md">Wilayah Operasional</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/analitik_kemitraan.php">
            <span class="material-symbols-outlined text-[20px]">analytics</span>
            <span class="text-label-md font-label-md">Analitik Kemitraan</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/financial_statements.php">
            <span class="material-symbols-outlined text-[20px]">payments</span>
            <span class="text-label-md font-label-md">Laporan Keuangan</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="/admin/settings.php">
    ...
    <div class="flex flex-col gap-xs py-md">
        <a class="flex items-center gap-sm px-md py-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-200" href="{{ route('home') }}">
            <span class="material-symbols-outlined text-[20px]">home</span>
            <span class="text-label-md font-label-md">Ke Beranda</span>
        </a>
        <a class="flex items-center gap-sm px-md py-sm text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 rounded-xl transition-all duration-200" href="/admin/logout_admin.php">
...
<main class="flex-grow flex flex-col h-screen overflow-hidden">
    ...
    <header class="sticky top-0 w-full z-40 flex justify-between items-center px-lg py-md bg-white border-b border-slate-100 max-w-none">
        <div class="flex items-center gap-md flex-1">
            <h1 class="text-headline-sm font-extrabold text-slate-800 text-[20px]">Kelola Pesanan Laundry (Laravel)</h1>
        </div>
        <div class="flex items-center gap-sm">
            <p class="text-label-md font-extrabold text-slate-800 leading-none">{{ Auth::user()->nama ?? 'Administrator' }}</p>
    ...
    <div class="flex-grow overflow-y-auto custom-scrollbar p-lg">
        <div class="max-w-7xl mx-auto space-y-lg">
            ...
            <!-- Alert Messages -->
            @if (session('success'))
                <div class="p-md bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl text-sm flex items-center gap-sm shadow-sm animate-fade-in">
            ...
            @if (session('error'))
                <div class="p-md bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl text-sm flex items-center gap-sm shadow-sm animate-fade-in">
            ...
            @if ($errors->any())
                <div class="p-md bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl text-sm shadow-sm">
                                ...
                                @foreach ($orders as $order) 
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="px-md py-md">
                                            <p class="font-bold text-slate-800">{{ $order->nama_pelanggan }}</p>
                                            @if (!empty($order->alamat_antar_jemput))
                                                <p class="text-xs text-slate-500 truncate max-w-[200px]" title="{{ $order->alamat_antar_jemput }}">
                                                    Alamat: {{ $order->alamat_antar_jemput }}
                                                </p>
                                            @endif
                                        </td>
                                        <td class="px-md py-md">
                                            <p class="font-bold text-slate-800">{{ $order->mitra->nama_mitra }}</p>
...
<div id="timbang-modal" class="fixed inset-0 z-[100] hidden bg-black/50 backdrop-blur-sm flex items-center justify-center p-md">
    <div class="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl relative flex flex-col">
        <div class="p-lg border-b border-slate-100 flex justify-between items-center">
            <h3 class="font-bold text-slate-800 text-lg">Input Timbangan Laundry (Laravel)</h3>
        ...
        <form method="POST" action="{{ route('admin.orders.timbang') }}" enctype="multipart/form-data" class="p-lg space-y-md">
            @csrf
...
<div id="status-modal" class="fixed inset-0 z-[100] hidden bg-black/50 backdrop-blur-sm flex items-center justify-center p-md">
    <div class="bg-white rounded-3xl max-w-sm w-full overflow-hidden shadow-2xl relative flex flex-col">
        <div class="p-lg border-b border-slate-100 flex justify-between items-center">
            <h3 class="font-bold text-slate-800 text-lg">Update Status Pesanan (Laravel)</h3>
        ...
        <form method="POST" action="{{ route('admin.orders.status') }}" class="p-lg space-y-md">
            @csrf
