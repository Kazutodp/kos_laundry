    ...
    <header class="w-full bg-surface-container shadow-sm py-4 px-6 border-b border-outline-variant/30">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <a class="flex items-center space-x-xs text-headline-md font-bold text-primary" href="{{ route('home') }}">
                <img alt="MataramWash Logo" class="h-8 w-8 object-contain" src="{{ asset('Logo_MataramWash.png') }}">
        ...
        <div class="max-w-md w-full bg-white rounded-3xl border border-outline-variant/20 p-xl shadow-[0px_10px_35px_rgba(0,0,0,0.06)] space-y-lg text-center relative overflow-hidden">
            ...
            <div class="bg-surface-container-low border border-outline-variant/30 rounded-2xl p-lg text-left space-y-md">
                <div class="flex justify-between border-b border-outline-variant/30 pb-sm">
                    ...
                    <span class="text-body-sm font-bold text-slate-700">#{{ $order->id }}</span>
                </div>

                <div class="space-y-sm text-body-sm">
                    <div class="flex justify-between">
                        <span class="text-slate-500">Mitra Laundry</span>
                        <span class="font-semibold text-on-surface">{{ $order->mitra->nama_mitra }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Layanan</span>
                        <span class="font-semibold text-on-surface">{{ $order->layanan }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Jumlah / Qty</span>
                        <span class="font-semibold text-on-surface">{{ floatval($order->berat_atau_qty) }} x Rp {{ number_format($order->tarif_per_kg, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between">
            ...
            <div class="p-md bg-sky-50 border border-sky-100 rounded-2xl flex items-start gap-sm text-left">
                ...
                <div class="text-[12px] text-sky-800 leading-normal">
                    <strong>Pemberitahuan:</strong> Kurir dari <strong>{{ $order->mitra->nama_mitra }}</strong> akan segera menghubungi Anda di nomor telepon terdaftar untuk koordinasi penjemputan cucian.
                </div>
            </div>

            <!-- Buttons -->
            <div class="pt-sm flex flex-col sm:flex-row gap-md justify-center">
                <a href="{{ route('home') }}" class="w-full sm:w-auto px-lg py-sm bg-primary text-white rounded-xl font-bold flex items-center justify-center gap-xs shadow-md hover:brightness-110 active:scale-95 transition-all text-sm">
                    <span class="material-symbols-outlined text-[18px]">home</span>
                    Beranda
                </a>
                <a href="#" class="w-full sm:w-auto px-lg py-sm bg-slate-100 text-slate-700 rounded-xl font-bold flex items-center justify-center gap-xs hover:bg-slate-200 active:scale-95 transition-all text-sm border border-slate-200">
