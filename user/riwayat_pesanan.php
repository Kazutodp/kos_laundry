...
try {
    $stmt = $pdo->prepare("
        SELECT o.*, m.nama_mitra, m.foto_toko 
        FROM orders o
        JOIN mitra_laundry m ON o.mitra_id = m.id
        WHERE o.nama_pelanggan = ?
        ORDER BY o.created_at DESC
    ");
    $stmt->execute([$user_nama]);
...
<head>
    ...
    <!-- Midtrans Snap.js -->
    <?php if ($config['midtrans_environment'] === 'production'): ?>
...
<body class="text-on-background bg-background min-h-screen flex flex-col justify-between">
    ...
    <main class="flex-grow max-w-4xl w-full mx-auto p-md py-xl">
        ...
        <?php if (empty($orders)): ?>
            <div class="bg-white rounded-3xl border border-outline-variant/20 p-xl text-center space-y-md shadow-sm">
                ...
                <a href="../index.php" class="inline-flex items-center gap-xs px-lg py-sm bg-primary text-white rounded-xl font-bold shadow-md hover:brightness-110 active:scale-95 transition-all text-sm mx-auto">
            ...
            <div class="space-y-lg">
                ...
                ?>
                    <div class="bg-white rounded-3xl border border-outline-variant/30 p-lg shadow-sm hover:shadow-md transition-all duration-200 flex flex-col md:flex-row gap-lg items-start">
                        <!-- Partner Thumbnail -->
                        <img alt="Outlet logo" class="w-16 h-16 rounded-2xl object-cover border border-slate-100 flex-shrink-0" src="../<?= htmlspecialchars($order['foto_toko'] ?: 'uploads/mitra_1.png'); ?>">
                        
                        <!-- Order Details -->
                        <div class="flex-1 space-y-md w-full">
                            <div class="flex flex-wrap justify-between items-start gap-xs">
                                <div>
                                    <h3 class="text-lg font-bold text-on-surface"><?= htmlspecialchars($order['nama_mitra']); ?></h3>
                                    <p class="text-sm text-primary font-semibold mt-base"><?= htmlspecialchars($order['layanan']); ?></p>
                                </div>
                                <div class="flex gap-xs items-center">
                                    ...
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold <?= $pay_badge_class; ?>">
                                        Pay: <?= htmlspecialchars(strtoupper($pay_status)); ?>
                                    </span>
                                </div>
                            </div>
                                ...
                                <div class="space-y-sm">
                                    ...
                                    <div>
                                        ...
                                        <span class="font-medium text-slate-800">
                                            ...
                                            <?php if (floatval($order['berat_atau_qty']) > 0): ?>
                                ...
                                <div class="space-y-sm">
                                    <div>
                                        ...
                                        <span class="font-medium text-slate-800">
                                            <?php 
                                                $distribution = [];
                                                if ($order['layanan_jemput'] ?? 1) $distribution[] = 'Jemput';
                                                if ($order['layanan_antar'] ?? 1) $distribution[] = 'Antar';
                                    ...
                                    <div>
                                        ...
                                        <span class="font-medium text-slate-800 truncate block max-w-xs" title="<?= htmlspecialchars($order['alamat_antar_jemput']); ?>">
                                            <?= htmlspecialchars($order['alamat_antar_jemput']); ?>
                                        </span>
                                    </div>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-md pt-sm border-t border-dashed border-outline-variant/30">
                                <div class="text-left">
                                    ...
                                    <?php if (!$is_self && floatval($order['berat_atau_qty']) > 0): ?>
                                ...
                                <div class="flex items-center gap-md">
                                    ...
                                    <?php if (!empty($order['foto_timbangan'])): ?>
                                        <button onclick="viewTimbangan('../<?= htmlspecialchars($order['foto_timbangan']); ?>')" class="flex items-center gap-xs text-xs font-bold text-secondary hover:text-emerald-700 transition-colors py-sm px-md border border-emerald-200 bg-emerald-50/50 rounded-xl shadow-xs">
    ...
    <script>
        ...
        function payOrder(orderId, btn) {
            ...
            fetch('generate_snap_token.php?order_id=' + orderId)
                ...
                if (data.status === 'success') {
                    ...
                    snap.pay(data.token, {
                        onSuccess: function(result) {
                            window.location.href = 'pembayaran_sukses.php?order_id=' + orderId + '&reference=' + result.order_id;
                        },
